...
StatementPtr Parser::parseFunctionDeclarationStatement() {
    ...
    expectToken(TokenType::LeftBrace, "Expected a '{' to introduce the function body");
    while (!done()) {
        if (currentToken().getType() == TokenType::RightBrace) {
            break;
        }
    ...
    expectToken(TokenType::RightBrace, "Expected a '}' to end the function body");
    return std::make_unique<ast::FunctionDeclarationStatement>(name, std::move(genericTypes), std::move(params), std::move(returnType), std::move(body));
}
...
StatementPtr Parser::parseIfStatement() {
    ...
    expectToken(TokenType::LeftBrace, "Expected '{' to start the if statement body");
    ast::Block body;
    while (!done()) {
        if (currentToken().getType() == TokenType::RightBrace) {
            break;  // End of the if body
        }
        body.push_back(parseStatement());
    }
    expectToken(TokenType::RightBrace, "Expected '}' to end the if statement body");
    ...
    while (currentToken().getType() == TokenType::Elif) {
        ...
        expectToken(TokenType::LeftBrace, "Expected '{' to start the elif statement body");
        ast::Block elifBody;
        while (!done()) {
            if (currentToken().getType() == TokenType::RightBrace) {
                break;  // End of the elif body
            }
            elifBody.push_back(parseStatement());
        }
        expectToken(TokenType::RightBrace, "Expected '}' to end the elif statement body");
        elifs.emplace_back(std::move(elifCondition), std::move(elifBody));
    }
    ast::Block elseBody;
    if (currentToken().getType() == TokenType::Else) {
        DISCARD(advance());

        expectToken(TokenType::LeftBrace, "Expected '{' to start the else statement body");
        while (!done()) {
            if (currentToken().getType() == TokenType::RightBrace) {
                break;  // End of the else body
            }
            elseBody.push_back(parseStatement());
        }
        expectToken(TokenType::RightBrace, "Expected '}' to end the else statement body");
    }
...
StatementPtr Parser::parseDoWhileLoopStatement() {
    DISCARD(advance());
    expectToken(TokenType::LeftBrace, "Expected '{' to start the do-while loop body");
    ast::Block body;
    while (!done()) {
        if (currentToken().getType() == TokenType::RightBrace) {
    ...
    expectToken(TokenType::RightBrace, "Expected '}' to end the do-while loop body");
...
StatementPtr Parser::parseWhileLoopStatement() {
    ...
    expectToken(TokenType::LeftBrace, "Expected '{' to start the while loop body");
    ast::Block body;
    while (!done()) {
        if (currentToken().getType() == TokenType::RightBrace) {
            break;  // End of the loop body
    ...
    expectToken(TokenType::RightBrace, "Expected '}' to end the while loop body");
    return std::make_unique<ast::WhileLoopStatement>(std::move(body), std::move(condition));
}
...
StatementPtr Parser::parseRepeatLoopStatement() {
    ...
    expectToken(TokenType::LeftBrace, "Expected '{' to start the repeat loop body");
    ast::Block body;
    while (!done()) {
        if (currentToken().getType() == TokenType::RightBrace) {
            break;  // End of the loop body
    ...
    expectToken(TokenType::RightBrace, "Expected '}' to end the repeat loop body");
    return std::make_unique<ast::RepeatLoopStatement>(std::move(numIterations), std::move(body));
}
