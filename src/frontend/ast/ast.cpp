...
bool BundleType::operator==(const Type& other) const noexcept {
    ...
    if (fieldTypes.size() != otherBundleType.fieldTypes.size()) {
        return false;
    }
    return std::equal(fieldTypes.begin(), fieldTypes.end(),
                      otherBundleType.fieldTypes.begin(), typePtrsEqual);
}
...
bool FunctionType::operator==(const Type& other) const noexcept {
    ...
    if (otherFunctionType.returnType.get() != returnType.get()) {
        return false;
    }
    if (otherFunctionType.parameterTypes.size() != parameterTypes.size()) {
        return false;
    }

    return std::equal(parameterTypes.begin(), parameterTypes.end(),
                      otherFunctionType.parameterTypes.begin());
}
...
bool GenericType::operator==(const Type& other) const noexcept {
    ...
    if (baseType.get() != otherGenericType.baseType.get()) {
        return false;
    }
    if (typeParameters.size() != otherGenericType.typeParameters.size()) {
        return false;
    }
    return std::equal(typeParameters.begin(), typeParameters.end(),
                    otherGenericType.typeParameters.begin(), typePtrsEqual);
}
