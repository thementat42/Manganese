...
void Lexer::tokenizeStringLiteral() {
    ...
    while (true) {
        ...
        if (peekChar() == '\\') {
            ...
            logging::logError(
                std::string("String literal cannot span multiple lines.") +
...
bool Lexer::processNumberSuffix(Base base, std::string& numberLiteral, bool isFloat) {
    ...
    if (currentChar == 'i' || currentChar == 'u' || currentChar == 'f') {
        ...
        if ((suffix[0] == 'i' || suffix[0] == 'u') && isFloat) {
            logging::logError("Integer suffix cannot be used with floating-point literals",
                              getLine(),
                              getCol());
            return false;
        }
    ...
    if (currentChar == 'e' && base == Base::Decimal) {
        ...
        if (isFloat) {
            ...
            if (!isdigit(currentChar)) {
                logging::logError("Invalid hexadecimal float: exponent must be a decimal number",
                                  getLine(),
                                  getCol());
                return false;
            }
