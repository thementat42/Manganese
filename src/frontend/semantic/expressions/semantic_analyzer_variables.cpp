...
constexpr lexer::TokenType getBinaryOperatorFromAssignmentOperator(lexer::TokenType assignmentOp) noexcept_if_release {
    ...
    switch (assignmentOp) {
        ...
        case BitXorAssign:
            return BitXor;
        case BitLShiftAssign:
            return BitLShift;
        case BitRShiftAssign:
            return BitRShift;
        default:
            ASSERT_UNREACHABLE(
                std::format("Cannot convert assignment operator {} to binary operator", lexer::tokenTypeToString(assignmentOp)));
            return Unknown;
    }
}
...
bool SemanticAnalyzer::handleInPlaceAssignment(Manganese::ast::AssignmentExpression* expression) {
    ...
    if (!areTypesCompatible(expression->assignee->getType(), tempBinaryExpression->getType())) {
        logError(
            "{} cannot be assigned to {}. {} has type {}, but {} has type {}", expression,
            expression->value->toString(), expression->assignee->toString(),
            expression->value->toString(), expression->value->getType()->toString(),
            expression->assignee->toString(), expression->assignee->getType()->toString());
        return false;
