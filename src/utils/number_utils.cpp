...
namespace utils {
using suffixMap_t = std::unordered_map<std::string, std::function<std::optional<number_t>(std::string_view, Base)>>;
...
template <typename T>
constexpr std::optional<T> _stox(std::string_view str, Base base = Base::Decimal)
    requires(std::is_integral_v<T> || std::is_floating_point_v<T>)
{
    T temp;
    const char* begin = str.data();
    const char* end = str.data() + str.size();
    auto [ptr, errorCode] = std::from_chars(begin, end, temp, static_cast<int>(base));
