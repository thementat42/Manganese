...
arg_parser.add_argument("-j", "--jobs", type=int, help="Number of parallel build jobs")
...
arg_parser.add_argument("--target", help="Specific CMake target to build")
...
if args.exec_with and not args.run:
    print(
...
if args.build_arg:
    build_args.append(
        "--"
    )  # tells cmake to treat all subsequent arguments as arguments to the build system
    # appended arguments are a list of list of strings (each sub-list contains 1 item)
    # so collect all those items
...
if not args.no_move:
    if bin_dir.exists() and output_file.exists():
        ...
        if dest_file.exists():
            print(
                f"Warning: {OUT_NAME} already exists in the root directory and will be overwritten"
            )
...
if args.run:
    ...
    print("Running with positional arguments:", *args.exec_with, "")
    run_command(cmd)
